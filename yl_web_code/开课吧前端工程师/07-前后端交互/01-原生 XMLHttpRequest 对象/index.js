...
router.get('/checkUserName', (ctx, next) => {
  ...
  if (res) {
    ctx.body = { status: 1, info: '用户名正确' };
  } else {
    ctx.body = { status: 2, info: '用户名错误' };
  }
});

router.get('/get/:id', (ctx, next) => {
  console.log(ctx.params);
  ctx.body = { status: 1, info: '请求成功' };
});

router.post('/post', (ctx, next) => {
  console.log(ctx.request.body);
  ctx.body = { status: 1, info: 'post请求成功' };
});

router.get('/xml', (ctx, next) => {
  // ctx.set('content-type', 'text/xml');  // 后端设置返回值的类型是xml
