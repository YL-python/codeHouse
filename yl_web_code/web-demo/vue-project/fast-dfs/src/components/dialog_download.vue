  ...
  <el-row :gutter="20">
    <el-col :span="24">
      <el-card class="box-card" v-loading="loading">
        ...
        <div style="text-align: center;">
          共
          <span style="font-size: 12px;color: red;">{{file.totalsize}}</span>；
          共
          <span style="font-size: 12px;color: red;">{{file.foldernum}}</span>个文件夹；
          共
          <span style="font-size: 12px;color: red;">{{file.filenum}}</span>个文件
        </div>
...
export default {
  ...
  methods: {
    ...
    getDownloadInfo() {
      this.loading = true;
      this.$http
        .post("disk/filedownload/getDownloadInfo", {
          idjson: this.idjson,
    ...
    download() {
      ...
      this.loading = true;
      this.$http
        .post("disk/filedownload/mergeFiles", {
