<template>
  <div style="height: 100%;">
      ...
      <el-col :span="4" class="nav" style="text-align: center;">
        ...
        <el-dropdown trigger="click" @command="handleCommand2">
          <span class="el-dropdown-link">
            <el-button
              type="text"
              v-if="search.orderfield=='filename'&&search.ordertype=='ascending'"
              icon="el-icon-top"
            >文件名称</el-button>
            <el-button
              type="text"
              v-if="search.orderfield=='filename'&&search.ordertype=='descending'"
              icon="el-icon-bottom"
            >文件名称</el-button>
            <el-button
              type="text"
              v-if="search.orderfield=='filesize'&&search.ordertype=='ascending'"
              icon="el-icon-top"
            >文件大小</el-button>
            <el-button
              type="text"
              v-if="search.orderfield=='filesize'&&search.ordertype=='descending'"
              icon="el-icon-bottom"
            >文件大小</el-button>
            <el-button
              type="text"
              v-if="search.orderfield=='createtime'&&search.ordertype=='ascending'"
              icon="el-icon-top"
            >创建日期</el-button>
            <el-button
              type="text"
              v-if="search.orderfield=='createtime'&&search.ordertype=='descending'"
              icon="el-icon-bottom"
            >创建日期</el-button>
          </span>
    ...
    <el-row style="height: 85%;overflow: auto;">
      <el-col :span="24">
        <div
          ...
          style="text-align: center;padding-top: 120px;font-size: 20px;color: #9b9b9b;"
        >
          ...
          >
            ...
            <el-dropdown-menu slot="dropdown" style="width: 120px;">
              <el-dropdown-item icon="el-icon-share">
                <el-button size="mini" type="text" @click="shareUI(item.id,item.filename)">分 享</el-button>
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-delete">
                <el-button size="mini" type="text" @click="del(item.id)">删 除</el-button>
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-document-copy">
                <el-button size="mini" type="text" @click="copyUI(item.id)">复制到</el-button>
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-document-remove">
                <el-button size="mini" type="text" @click="moveUI(item.id)">移动到</el-button>
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-edit">
                <el-button size="mini" type="text" @click="renameUI(item.id)">重命名</el-button>
              </el-dropdown-item>
              <el-dropdown-item icon="el-icon-postcard">
                <el-button size="mini" type="text" @click="propUI(item.id,item.filetype)">属 性</el-button>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <!--文字-->
          <div
            style="font-size: 12px;color: #409EFF;background: #F2F6FC;padding: 5px;box-sizing: border-box;"
          >
            <span
    ...
    >
      <createdocument
        ref="form"
        v-if="dialog.type=='createdocument'"
        v-on:closeAndRefresh="createDocumentClose"
      ></createdocument>
      <createfolder
        ref="form"
        v-if="dialog.type=='createfolder'"
        v-on:closeAndRefresh="createFolderClose"
      ></createfolder>
            ...
            if (rows != null && rows.length != 0) {
              for (var i = 0; i < rows.length; i++) {
                ...
                if (d.typecode == "picture") {
                  d.icon =
                    this.baseurl +
                    "disk/fileopen/previewThumbnail?url=" +
                    d.thumbnailurl +
                    "&token=" +
